using Libraries.Standard_Assets.Effects.ImageEffects.Scripts;
using UnityEditor;
using UnityEngine;

namespace Libraries.Standard_Assets.Editor.ImageEffects
{
    ...
    class BloomAndFlaresEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUILayout.Label(
                "HDR " + (hdr.enumValueIndex == 0 ? "auto detected, " : (hdr.enumValueIndex == 1 ? "forced on, " : "disabled, ")) +
                (useSrcAlphaAsMask.floatValue < 0.1f ? " ignoring alpha channel glow information" : " using alpha channel glow information"),
            ...
            if (1 == tweakMode.intValue)
                useSrcAlphaAsMask.floatValue = EditorGUILayout.Slider(new GUIContent("Use alpha mask", "Make alpha channel define glowiness"),
                ...
                if (lensflares.boolValue)
                {
                    ...
                    else if (lensflareMode.intValue == 1)
                    {
                        ...
                        hollywoodFlareBlurIterations.intValue =
                            EditorGUILayout.IntSlider("Blur iterations", hollywoodFlareBlurIterations.intValue, 1, 4);
