    ...
    public class CameraMotionBlur : PostEffectsBase
    {
        ...
        private void CalculateViewProjection()
        {
            ...
            if (_camera.stereoEnabled)
            {
                for (int eye = 0; eye < 2; ++eye)
                {
                    ...
                    Matrix4x4 stereoProjMat =
                        _camera.GetStereoProjectionMatrix(eye == 0 ? Camera.StereoscopicEye.Left : Camera.StereoscopicEye.Right);
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            if (filterType == MotionBlurFilter.CameraMotion)
            {
                ...
                farHeur = (Vector3.Angle(transform.forward, prevFrameForward) / _camera.fieldOfView) * (source.width * 0.75f);
                blurVector.z = rotationScale * (1.0f - lookUpDown) *
